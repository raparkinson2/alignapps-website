...
export default function AddGameModal({ isOpen, onClose, existingGame }: AddGameModalProps) {
  ...
  const activePlayers = players.filter((p: Player) => p.status === 'active');
  ...
  const handleSave = async () => {
    ...
    try {
      const game: Game = {
        ...
        beerDutyPlayerId: form.showBeerDuty && form.beerDutyPlayerId ? form.beerDutyPlayerId : undefined,
      ...
      <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2">
              {teamSettings.jerseyColors.map((jc) => (
                <button
                  ...
                  className={cn(
                    'flex items-center gap-3 w-full px-4 py-2.5 rounded-xl border text-sm transition-all text-left',
                ...
                >
                  ...
                  {form.jerseyColor === jc.name && (
                    <span className="ml-auto text-[#67e8f9]">✓</span>
                  )}
                </button>
              ))}
            </div>
        ...
        {teamSettings.showRefreshmentDuty && (
          <div>
            ...
            {form.showBeerDuty && (
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1.5">Who has duty? (optional)</label>
                <select
                  value={form.beerDutyPlayerId}
                  onChange={(e) => set('beerDutyPlayerId', e.target.value)}
                  className="w-full bg-white/[0.05] border border-white/10 rounded-xl px-4 py-2.5 text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#67e8f9]/40 text-sm"
                >
                  <option value="">— Not assigned —</option>
                  {activePlayers.map((p: Player) => (
                    <option key={p.id} value={p.id}>
                      {p.firstName} {p.lastName}{p.number ? ` #${p.number}` : ''}
                    </option>
                  ))}
                </select>
              </div>
